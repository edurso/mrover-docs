...
- Linux x86_64, on a non-Ubuntu distro (Arch, Fedora, etc. — if you're on Ubuntu, just use the [native install](/general-resources/ros/installing-the-codebase) instead)
...
You'll also need an SSH key set up with Github to clone — see the [native install guide](/general-resources/ros/installing-the-codebase#set-up-your-ssh-key) if you haven't done that yet.
...
Same as the [native install](/general-resources/ros/installing-the-codebase#sanity-check) — run `mrover` then `ros2 launch mrover simulator.launch.py` and confirm the simulator comes up.
